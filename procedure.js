	...
	MecaniqueWord.prototype.update = function(){
		...
		if( j % 2 == 0 ){
			...
			if( k == indexDansSeg ){
				...
				// on découpe la partie déjà acceptée de correct en block séparés par des espaces
...
function FrappeCoordinator ( ) {
	...
	this.startFurySession = function (){
		...
		// on définit les paramétres aléatoires
		var possibleFuryKey = [ "a" , "z", 'w' ];
...
function Engine( ) {
	...
	this.end = function (){
		...
		// on lance la derniére animation
		animateur.popEnd();
...
var ecouteurClavier = function( event ){
	...
	try {
		...
		if( event.which == 32 || ( 65 <= event.which && event.which <= 90 ) ){
			frappeCoord.accepteChar( key_tab[ event.which ] );
			return;
		}
